...
import { Shield, Sparkles, Wrench, ArrowRight, CheckCircle2, AlertCircle } from 'lucide-react';
...
export const LoginView: React.FC<LoginViewProps> = ({ onLoginSuccess }) => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      setErrorMessage(
        err?.message || 'Chưa cấu hình Firebase Auth Google Login. Bạn vẫn có thể trải nghiệm ngay bằng nút Dùng thử Demo bên dưới!'
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#0D2B2B] via-[#1A3D2B] to-[#2D7D46] text-white flex flex-col justify-between p-4 sm:p-6 relative overflow-hidden">
      ...
      <div className="max-w-md mx-auto w-full my-6 bg-black/20 border border-emerald-500/20 rounded-2xl p-4 backdrop-blur-sm relative z-10 space-y-2 text-xs text-emerald-100">
        <div className="flex items-center gap-2">
          ...
          <span>Tự động tính Chỉ số Nguy cơ Cadmium (CRS) theo gian thực</span>
        </div>
        <div className="flex items-center gap-2">
          <CheckCircle2 className="w-4 h-4 text-[#FFC107] shrink-0" />
      ...
      <div className="max-w-md mx-auto w-full space-y-3 relative z-10 mb-6">
        ...
        {errorMessage && (
          <div className="bg-amber-900/80 border border-amber-500 text-amber-100 text-xs p-3 rounded-2xl flex items-start gap-2">
            <AlertCircle className="w-4 h-4 text-amber-300 shrink-0 mt-0.5" />
            <span>{errorMessage}</span>
          </div>
        )}

        {/* Option 2: Demo Mode (Highlight for farmers) */}
        <button
          onClick={handleDemoLogin}
          className="w-full bg-[#FFC107] hover:bg-amber-400 text-slate-950 font-black py-4 px-5 rounded-2xl shadow-xl border-2 border-amber-300 flex items-center justify-between gap-3 transition-all active:scale-[0.98] text-base"
        >
          <div className="flex items-center gap-3">
            <span className="text-2xl">🌿</span>
            <div className="text-left">
              <span className="block font-black text-sm sm:text-base text-slate-950">XEM TRẠNG THÁI VƯỜN NGAY</span>
              <span className="block text-[11px] text-slate-800 font-extrabold">Không cần mật khẩu – Dành cho nông dân</span>
            </div>
          </div>
        ...
        <button
          ...
          disabled={loading}
          className="w-full bg-white/95 hover:bg-white text-[#1F2D24] font-extrabold py-3.5 px-4 rounded-2xl shadow-md flex items-center justify-center gap-3 transition-all active:scale-[0.98] border border-emerald-200 mt-2"
        >
          ...
          <span className="text-sm">Đăng nhập tài khoản Google</span>
        </button>

        {/* Option 3: Dev Mode */}
        <button
          onClick={handleDevModeLogin}
          className="w-full bg-[#0D2B2B]/80 hover:bg-[#0D2B2B] text-amber-300 font-extrabold py-3.5 px-4 rounded-2xl border border-amber-500/40 flex items-center justify-center gap-2 transition-all active:scale-[0.98]"
        >
          <Wrench className="w-5 h-5 text-amber-400" />
          <span className="text-sm">Demo có thể chỉnh – Dev Mode</span>
